...
def make_ccd_qa(
    ...
    if radius_pixel is None:
        radius_pixel = legacyhalos.misc.cutout_radius_kpc(
            redshift=onegal[zcolumn], pixscale=pixscale, radius_kpc=radius_mosaic_kpc
        )  # [pixels]
...
def make_multiwavelength_coadds(
    ...
    for bandsuffix, pixscale in zip(("FUVNUV", "W1W2"), (1.5, 2.75)):
        ...
        thumb2file = os.path.join(
            htmlgalaxydir,
            "thumb2-{}-{}-montage-{}.png".format(galaxy, filesuffix, bandsuffix),
        )
        if not os.path.isfile(montagefile) or clobber:
            coaddfiles = (
                "{}-image-{}".format(filesuffix, bandsuffix),
        ...
        if bool(ellipsefit):
            ...
            if galex or unwise:
                sedfile = os.path.join(
                    htmlgalaxydir,
                    "{}-{}-ellipse-{}sed.png".format(galaxy, data["filesuffix"], galid),
                )
            ...
            cogfile = os.path.join(
                htmlgalaxydir,
                "{}-{}-ellipse-{}cog.png".format(galaxy, data["filesuffix"], galid),
            )
            ...
            if unwise:
                ...
                if not os.path.isfile(multibandfile) or clobber:
                    with Image.open(
                        os.path.join(
                            galaxydir,
                            "{}-{}-image-W1W2.jpg".format(galaxy, data["filesuffix"]),
                        )
                    ) as colorimg:
                        display_multiband(
                            data,
            ...
            if galex:
                ...
                if not os.path.isfile(multibandfile) or clobber:
                    with Image.open(
                        os.path.join(
                            galaxydir,
                            "{}-{}-image-FUVNUV.jpg".format(galaxy, data["filesuffix"]),
                        )
                    ) as colorimg:
                        display_multiband(
                            data,
            ...
            if not os.path.isfile(multibandfile) or clobber:
                with Image.open(
                    os.path.join(
                        galaxydir,
                        "{}-{}-image-grz.jpg".format(galaxy, data["filesuffix"]),
                    )
                ) as colorimg:
                    display_multiband(
                        data,
