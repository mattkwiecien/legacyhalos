...
def get_galaxy_galaxydir(cat, datadir=None, htmldir=None, html=False, candidates=False):
    ...
    galaxydir = np.array(
        [os.path.join(datadir, str(pix), gal) for pix, gal in zip(pixnum, galaxy)]
    )
    ...
    if html:
        ...
        htmlgalaxydir = np.array(
            [os.path.join(htmldir, str(pix), gal) for pix, gal in zip(pixnum, galaxy)]
        )
...
def missing_files(args, sample, size=1, clobber_overwrite=None):
    ...
    for gal, gdir in zip(np.atleast_1d(galaxy), np.atleast_1d(galaxydir)):
        ...
        if dependson:
            missargs.append(
                [checkfile, os.path.join(gdir, "{}{}".format(gal, dependson)), clobber]
            )
        else:
...
def mpi_args():
    ...
    parser.add_argument(
        "--sdss", action="store_true", help="Analyze the SDSS galaxies."
    )
    ...
    parser.add_argument(
        "--coadds", action="store_true", help="Build the coadds with the custom sky."
    )
    parser.add_argument(
        "--pipeline-coadds",
        action="store_true",
        help="Build the coadds with the pipeline sky.",
    )
    parser.add_argument(
        "--just-coadds",
        action="store_true",
        help="Just build the pipeline coadds and return (using --early-coadds in runbrick.py).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Random seed (used with --sky and --sersic).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.396,
        type=float,
        help="SDSS pixel scale (arcsec/pix).",
    )
...
def get_lambdabins(verbose=False):
    ...
    if verbose:
        for ii in range(nlbins - 1):
            print(
                "Bin {}: lambda={:03d}-{:03d}, Mhalo={:.3f}-{:.3f} Msun".format(
                    ii,
                    lambdabins[ii].astype("int"),
                    lambdabins[ii + 1].astype("int"),
                    mhalobins[ii],
                    mhalobins[ii + 1],
                )
            )
...
def get_zbins(zmin=0.1, zmax=0.3, dt=0.5, verbose=False):
    ...
    if verbose:
        for ii in range(ntbins - 1):
            print(
                "Bin {}: z={:.3f}-{:.3f}, t={:.3f}-{:.3f} Gyr, dt={:.3f} Gyr".format(
                    ii,
                    zbins[ii],
                    zbins[ii + 1],
                    tbins[ii],
...
def cutout_radius_cluster(
    ...
    radius = np.rint(
        factor * radius_kpc * cosmo.arcsec_per_kpc_proper(redshift).value / pixscale
    )
...
def read_redmapper(
    ...
    cenfile = os.path.join(
        os.getenv("REDMAPPER_DIR"),
        rmversion,
    ...
    cen[DIAMCOLUMN] = cutout_radius_kpc(
        redshift=cen[ZCOLUMN],
        cosmo=cosmo,  # diameter [arcsec]
        radius_kpc=2 * RADIUS_CLUSTER_KPC,
    )
    cen[GALAXYCOLUMN] = np.array(
        [
            "{:07d}-{:09d}".format(mid, cid)
...
def _build_multiband_mask(
    ...
    for ii, central in enumerate(galaxy_indx):
        ...
        )  # , plot=True) ; plt.savefig('debug.png')
        # if True:
        #    import matplotlib.pyplot as plt
        ...
        if False:
            ...
            plt.clf()
            mgegalaxy = find_galaxy(img, nblob=1, binning=1, quiet=True, plot=True)
        ...
        for filt in bands:
            ...
            satmask = np.logical_or(
                satmask, satimg > 10 * data["{}_sigma".format(filt)]
            )
            # if True:
            #    import matplotlib.pyplot as plt
...
def read_multiband(
    ...
    for galid in np.atleast_1d(galaxy_id):
        ...
        if len(galindx) != 1:
            raise ValueError(
                "Problem finding the central galaxy {} in the tractor catalog!".format(
                    galid
                )
            )
        ...
        for filt in bands:
            ...
            if cenflux <= 0.0 or cenivar <= 0.0:
                print(
                    "Central galaxy flux is negative. Off footprint or gap in coverage?"
                )
                data["failed"] = True
                return data, []
    ...
    if sky_tests:
        ...
        if logfile:
            ...
            if bool(data):
                ...
                if os.path.isfile(
                    os.path.join(
                        galaxydir, "{}-{}-coadds.isdone".format(galaxy, filesuffix)
                    )
                ):
                    err = 1  # successful failure
                else:
...
def read_integrated_flux(first=None, last=None, integratedfile=None, verbose=False):
    ...
    if not os.path.isfile(integratedfile):
        print(
            "File {} not found.".format(integratedfile)
        )  # non-catastrophic error is OK
        return None
...
def write_results(
    ...
    sersic_single=None,
    sersic_double=None,
    sersic_exponential=None,
    sersic_single_nowavepower=None,
    sersic_double_nowavepower=None,
    sersic_exponential_nowavepower=None,
    clobber=False,
    verbose=False,
):
    ...
    if not os.path.isfile(resultsfile) or clobber:
        ...
        for tt, name in zip(
            (
                ...
                sersic_double,
                sersic_exponential,
                sersic_single_nowavepower,
                sersic_double_nowavepower,
...
def read_results(first=None, last=None, verbose=False, extname="RESULTS", rows=None):
    ...
    if not os.path.isfile(resultsfile):
        ...
        if verbose:
            print(
                "Read {} objects from {} [{}]".format(
                    len(results), resultsfile, extname
                )
            )
        return results
...
def _read_paper_sample(
    paper="profiles",
    first=None,
    last=None,
    dr="dr8",
    sfhgrid=1,
    isedfit_lsphot=False,
    isedfit_sdssphot=False,
    isedfit_lhphot=False,
    candidates=False,
    kcorr=False,
    verbose=False,
):
    ...
    if isedfit_lsphot:
        samplefile = os.path.join(
            paperdir,
            "{}-{}-sfhgrid{:02d}-lsphot-{}.fits".format(paper, prefix, sfhgrid, dr),
        )
    elif isedfit_sdssphot:
        samplefile = os.path.join(
            paperdir,
            "{}-{}-sfhgrid{:02d}-sdssphot-dr14.fits".format(paper, prefix, sfhgrid),
        )
    elif isedfit_lhphot:
        samplefile = os.path.join(
            paperdir, "{}-{}-sfhgrid{:02d}-lhphot.fits".format(paper, prefix, sfhgrid)
        )
    else:
        samplefile = os.path.join(
            paperdir, "sample-{}-{}-{}.fits".format(paper, prefix, dr)
        )
...
def read_smf_sample(
    ...
    sample = _read_paper_sample(
        paper="smf",
        first=first,
        last=last,
        dr=dr,
        sfhgrid=1,
        isedfit_lsphot=isedfit_lsphot,
        isedfit_sdssphot=isedfit_sdssphot,
        isedfit_lhphot=isedfit_lhphot,
        kcorr=kcorr,
...
def literature(kravtsov=True, gonzalez=False):
    ...
    if kravtsov:
        krav = dict()
        krav["m500c"] = np.log10(
            np.array([15.6, 10.3, 7, 5.34, 2.35, 1.86, 1.34, 0.46, 0.47]) * 1e14
        )
        krav["mbcg"] = (
        ...
        krav["mbcg_err"] = (
            np.array([0.36, 0.3, 0.3, 0.13, 0.05, 0.11, 0.06, 0.05, 0.14]) * 1e12
        )
        krav["mbcg_err"] = krav["mbcg_err"] / krav["mbcg"] / np.log(10)
        krav["mbcg"] = np.log10(krav["mbcg"])

        M200c, _, _ = mass_defs.changeMassDefinition(
            10 ** krav["m500c"], 3.5, 0.0, "500c", "200c"
        )
    ...
    if gonzalez:
        ...
        gonz["mbcg"] = (
            np.array(
                [0.84, 0.87, 0.33, 0.57, 0.85, 0.60, 0.86, 0.93, 0.71, 0.81, 0.70, 0.57]
            )
            * 1e12
            * 2.65
        )
        ...
        gonz["mbcg_err"] = (
            np.array(
                [0.03, 0.09, 0.01, 0.01, 0.14, 0.03, 0.03, 0.05, 0.07, 0.12, 0.02, 0.01]
            )
            * 1e12
            * 2.65
        )
        ...
        gonz["m500c_err"] = (
            np.array(
                [0.19, 0.42, 0.1, 0.32, 0.28, 0.11, 0.1, 0.19, 0.23, 0.18, 0.24, 0.21]
            )
            * 1e14
        )
...
def obsolete_make_html(
    ...
    galaxy, galaxydir, htmlgalaxydir = legacyhalos.io.get_galaxy_galaxydir(
        sample, html=True
    )
    ...
    def _viewer_link(gal):
        ...
        width = 2 * legacyhalos.misc.cutout_radius_kpc(
            redshift=gal[zcolumn], pixscale=0.262, radius_kpc=radius_mosaic_kpc
        )  # [pixels]
        if width > 400:
            zoom = 14
        else:
    ...
    def _skyserver_link(objid):
        return "http://skyserver.sdss.org/dr14/en/tools/explore/summary.aspx?id={:d}".format(
            objid
        )
    ...
    with open(homehtmlfile, "w") as html:
        ...
        html.write("</tr>\n")
        for ii, (gal, galaxy1, htmlgalaxydir1) in enumerate(
            zip(sample, np.atleast_1d(galaxy), np.atleast_1d(htmlgalaxydir))
        ):
    ...
    for ii, (gal, galaxy1, galaxydir1, htmlgalaxydir1) in enumerate(
        ...
        radius_mosaic_arcsec = legacyhalos.misc.cutout_radius_kpc(
            redshift=gal[zcolumn], radius_kpc=radius_mosaic_kpc
        )  # [arcsec]
        ...
        with open(htmlfile, "w") as html:
            ...
            html.write("<br />\n")
            html.write(
                '<a href="../../../../{}">Next Central Galaxy ({})</a>\n'.format(
                    nexthtmlgalaxydir1, nextgalaxy[ii]
                )
            )
            html.write("<br />\n")
            html.write(
                '<a href="../../../../{}">Previous Central Galaxy ({})</a>\n'.format(
            ...
            html.write(
                "<p>Each mosaic (left to right: data, model of all but the central galaxy, and residual image containing just the central galaxy) is {:.0f} kpc = {:.3f} arcsec = {:.0f} pixels in diameter.</p>\n".format(
                    2 * radius_mosaic_kpc, 2 * radius_mosaic_arcsec, diam_mosaic_pixels
                )
            )
            # html.write('<br />\n')
            ...
            html.write(
                '<tr><td><a href="{}-grz-montage.png"><img src="{}-grz-montage.png" alt="Missing file {}-grz-montage.png" height="auto" width="100%"></a></td></tr>\n'.format(
                    galaxy1, galaxy1, galaxy1
                )
            )
            ...
            if False:
                ...
                html.write("<tr>\n")
                html.write(
                    '<td><a href="{}-sersic-single-nowavepower.png"><img src="{}-sersic-single-nowavepower.png" alt="Missing file {}-sersic-single-nowavepower.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write(
                    '<td><a href="{}-sersic-single.png"><img src="{}-sersic-single.png" alt="Missing file {}-sersic-single.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write("</tr>\n")
                ...
                html.write("<tr>\n")
                html.write(
                    '<td><a href="{}-sersic-exponential-nowavepower.png"><img src="{}-sersic-exponential-nowavepower.png" alt="Missing file {}-sersic-exponential-nowavepower.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write(
                    '<td><a href="{}-sersic-exponential.png"><img src="{}-sersic-exponential.png" alt="Missing file {}-sersic-exponential.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write("</tr>\n")
                ...
                html.write("<tr>\n")
                html.write(
                    '<td><a href="{}-sersic-double-nowavepower.png"><img src="{}-sersic-double-nowavepower.png" alt="Missing file {}-sersic-double-nowavepower.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write(
                    '<td><a href="{}-sersic-double.png"><img src="{}-sersic-double.png" alt="Missing file {}-sersic-double.png" height="auto" width="100%"></a></td>\n'.format(
                        galaxy1, galaxy1, galaxy1
                    )
                )
                html.write("</tr>\n")
        ...
        if False:  # html_raslices:
            ...
            for gal, galaxy1, htmlgalaxydir1 in zip(
                ...
                pngfile1 = os.path.join(
                    ...
                    "{}-custom-grz-montage.png".format(galaxy1),
                )
                thumbfile1 = os.path.join(
                    htmlgalaxydir1.replace(htmldir, "")[1:],
                    "thumb2-{}-custom-grz-montage.png".format(galaxy1),
                )
...
def build_htmlpage_one(
    ...
    def _read_ccds_tractor_sample(prefix):
        ...
        samplefile = os.path.join(
            galaxydir1, "{}-redmapper-sample.fits".format(galaxy1)
        )
        ...
        if os.path.isfile(tractorfile):
            ...
            if prefix == "custom":
                ...
                if len(wt) == 0:
                    ...
                    assert len(wt) == len(
                        ws
                    )  # there are duplicate satellites in tractor!
    ...
    def _html_cluster_properties(html, gal):
        ...
        ra1, dec1, diam1 = gal[racolumn], gal[deccolumn], gal[diamcolumn]
        viewer_link = legacyhalos.html.viewer_link(
            ra1, dec1, diam1 * 2 * 60 / pixscale, sga=False
        )
    ...
    def _html_image_mosaics(html):
        ...
        pngfile, thumbfile = "{}-custom-grz-montage.png".format(
            galaxy1
        ), "thumb-{}-custom-grz-montage.png".format(galaxy1)
        html.write(
        ...
        for filesuffix in ["custom-grz"]:  # , 'W1W2'):
            pngfile, thumbfile = "{}-{}-montage.png".format(
                galaxy1, filesuffix
            ), "thumb-{}-{}-montage.png".format(galaxy1, filesuffix)
            html.write(
    ...
    def _html_ellipsefit_and_photometry(html, tractor, sample):
        ...
        for igal in np.arange(len(tractor["ref_id"])):
            ...
            ellipse = legacyhalos.io.read_ellipsefit(
                galaxy1, galaxydir1, filesuffix="custom", verbose=verbose, galaxyid=""
            )
            if not bool(ellipse):
