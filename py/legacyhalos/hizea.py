...
def read_sample(first=None, last=None, galaxylist=None, verbose=False, columns=None):
    ...
    if True:
        ...
        sample = astropy.table.Table(
            info[ext].read(rows=rows, upper=True, columns=columns)
        )
        if verbose:
            if len(rows) == 1:
...
def build_catalog(sample, nproc=1, verbose=False):
    ...
    outfile = os.path.join(
        legacyhalos.io.legacyhalos_data_dir(), "hizea-legacyphot.fits"
    )
    ...
    for gal, gdir, onegal in zip(galaxy, galaxydir, sample):
        ...
        if os.path.isfile(tractorfile):  # just in case
            ...
            if len(m1) != 1:
                print(
                    "Multiple matches within 1 arcsec for {}".format(
                        onegal["GALAXY_FULL"]
                    )
                )
                m1 = m1[0]
...
def _build_multiband_mask(
    ...
    for ii, central in enumerate(galaxy_indx):
        ...
        centralmask[
            np.logical_and(data["starmask"], np.logical_not(centralmask2))
        ] = False
        ...
        for filt in bands:
            ...
            if psfsrcs:
                ...
                if False:
                    ...
                    plt.savefig(
                        "desi-users/ioannis/tmp/qa-psf-{}.png".format(filt.lower())
                    )
                    if filt == "r":  # or filt == 'r':
                        pdb.set_trace()
                img -= psfimg
...
def read_multiband(
    ...
    for band in bands:
        cols = cols + [
            "flux_{}".format(band.lower()),
            "flux_ivar_{}".format(band.lower()),
        ]
        cols = cols + ["mw_transmission_{}".format(band.lower())]
    for band in optbands:
        cols = cols + [
            "nobs_{}".format(band.lower()),
            "psfdepth_{}".format(band.lower()),
...
def build_htmlpage_one(
    ...
    def _html_image_mosaics(html):
        ...
        for filesuffix in ("FUVNUV", "grz", "W1W2"):
            pngfile, thumbfile = "{}-custom-montage-{}.png".format(
                galaxy1, filesuffix
            ), "thumb-{}-custom-montage-{}.png".format(galaxy1, filesuffix)
            html.write(
    ...
    def _html_ellipsefit_and_photometry(html, tractor, sample):
        ...
        html.write("<tr><th>Galaxy</th>\n")
        html.write(
            "<th>FUV</th><th>NUV</th><th>g</th><th>r</th><th>i</th><th>z</th><th>W1</th><th>W2</th><th>W3</th><th>W4</th>\n"
        )
        html.write(
            "<th>FUV</th><th>NUV</th><th>g</th><th>r</th><th>i</th><th>z</th><th>W1</th><th>W2</th><th>W3</th><th>W4</th>\n"
        )
