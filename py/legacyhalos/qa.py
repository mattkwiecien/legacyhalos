...
def draw_ellipse_on_png(
    im,
    x0,
    y0,
    ba,
    pa,
    major_axis_diameter_arcsec,
    pixscale,
    color="#3388ff",
    linewidth=3,
):
...
def addbar_to_png(
    ...
    with Image.open(jpgfile) as im:
        ...
        if barlen:
            ...
            x0, x1, y0, y1 = (
                0 + fntsize * 2,
                0 + fntsize * 2 + barlen * pixscalefactor,
                sz[1] - fntsize * 1.5,
                sz[1] - fntsize * 4,
            )
...
def qa_maskbits(mask, tractor, ellipsefitall, colorimg, largegalaxy=False, png=None):
    ...
    for igal, ellipsefit in enumerate(ellipsefitall):
        ...
        ragal, decgal, pa, ba = (
            ellipsefit["ra_moment"],
            ellipsefit["dec_moment"],
        ...
        if igal == 0:
            ax3.scatter(
                tractor["BX"][inellipse],
                tractor["BY"][inellipse],
                s=ss,
                marker="s",
                edgecolor="k",
        ...
        draw_ellipse_on_png(
            ...
            imgsz[1] - ellipsefit["y0_moment"],
            1 - ellipsefit["eps_moment"],
            ellipsefit["pa_moment"],
            2 * maxis * ellipsefit["refpixscale"],
            ellipsefit["refpixscale"],
...
def qa_curveofgrowth(
    ellipsefit,
    pipeline_ellipsefit=None,
    png=None,
    plot_sbradii=False,
    cosmo=None,
    verbose=True,
    ...
    if plot_sbradii:
        ...
        if False:
            ll = ax.axvline(
                x=ellipsefit["majoraxis"] * ellipsefit["refpixscale"],
                lw=2,
                color="#e41a1c",
                ls="dotted",
            )
    ...
    for band, lam in zip(sersic["bands"], (sersic["lambda_g"], sersic["lambda_r"], sersic["lambda_z"])):
        ...
        if model is not None:
            ...
            if False:
                ...
                sb_model2 = SersicSingleWaveModel(
                    seed=model.seed,
                    psfsigma_g=model.psfsigma_g * 0,
                    psfsigma_r=model.psfsigma_r * 0,
                    psfsigma_z=model.psfsigma_z * 0,
                    pixscale=model.pixscale,
                ).evaluate(
                    # rad, wave,
                    rad_model,
                    wave2,
            ...
            if model.__class__.__name__ == "SersicDoubleWaveModel" and band == "r" and 0 == 1:
                ...
                model1 = SersicSingleWaveModel(
                    nref=model.nref1.value,
                    r50ref=model.r50ref1.value,
                    alpha=model.alpha1.value,
                    beta=model.beta1.value,
                    mu50_g=model.mu50_g1.value,
                    mu50_r=model.mu50_r1.value,
                    mu50_z=model.mu50_z1.value,
                )
                model2 = SersicSingleWaveModel(
                    nref=model.nref2.value,
                    r50ref=model.r50ref2.value,
                    alpha=model.alpha2.value,
                    beta=model.beta2.value,
                    mu50_g=model.mu50_g2.value,
                    mu50_r=model.mu50_r2.value,
                    mu50_z=model.mu50_z2.value,
                )
                ax.plot(
                    rad_model,
                    22.5 - 2.5 * np.log10(model1(rad_model, wave_model)),
                    color="gray",
                    alpha=0.5,
                    ls="-.",
                    lw=2,
                ...
                ax.plot(
                    rad_model,
                    22.5 - 2.5 * np.log10(model2(rad_model, wave_model)),
                    color="gray",
                    alpha=0.5,
                    ls="-.",
                    lw=2,
...
def display_multiband(
    ...
    if ellipsefit and ellipsefit["success"]:
        ...
        if False:
            ...
            draw_ellipse_on_png(
                ...
                1 - ellipsefit["eps_moment"],
                ellipsefit["pa_moment"],
                2 * ellipsefit["majoraxis"] * ellipsefit["refpixscale"],
                ellipsefit["refpixscale"],
    ...
    for ii, (filt, ax1) in enumerate(zip(bands, ax[1:])):
        ...
        if (
            ...
            and np.atleast_1d(ellipsefit["sma_{}".format(filt.lower())])[0] != -1
            and len(smas) > 1
        ):
            ...
            for sma in smas:
                ...
                ax1.add_patch(
                    mpatches.Ellipse(
                        (
                            ellipsefit["x0_{}".format(filt.lower())][this],
                            ellipsefit["y0_{}".format(filt.lower())][this],
                        ),
                        2 * ellipsefit["sma_{}".format(filt.lower())][this],
                        2
                        * ellipsefit["sma_{}".format(filt.lower())][this]
                        * (1 - ellipsefit["eps_{}".format(filt.lower())][this]),
            ...
            if maxis > 0:  # and galex is False and unwise is False:
                ellaper = EllipticalAperture(
                    (
                        ellipsefit["x0_moment"] * pixscalefactor,
                        ellipsefit["y0_moment"] * pixscalefactor,
                    ),
                    maxis * pixscalefactor,
                    maxis * (1 - ellipsefit["eps_moment"]) * pixscalefactor,
                    np.radians(ellipsefit["pa_moment"] - 90),
                )
...
def display_ellipsefit(ellipsefit, xlog=False, cosmo=None, png=None, verbose=True):
    ...
    if ellipsefit["success"]:
        ...
        if np.count_nonzero(bad) > 0:
            ax1.scatter(
                ellipsefit[refband]["sma"][bad] * refpixscale,
...
def display_ccdpos(onegal, ccds, radius, grzfile, pixscale=0.262, png=None, verbose=False):
    ...
    bb_clust, bbcc_clust = (
        wcs_clust.radec_bounds(),
        wcs_clust.radec_center(),
    )  # [degrees]
    ...
    for ax, band in zip(allax, ("g", "r", "z")):
        ...
        ax.add_patch(
            patches.Rectangle(
                (bbcc_clust[0] - radius_deg, bbcc_clust[1] - radius_deg),
                radius_deg * 2,
                radius_deg * 2,
                fill=False,
                edgecolor="black",
        ...
        if False:
            ax.add_patch(
                patches.Circle(
                    (bbcc_clust[0], bbcc_clust[1]),
                    1.1 * radius_deg,  # inner sky annulus
                    fill=False,
                    edgecolor="gray",
                    lw=1,
                )
            ...
            ax.add_patch(
                patches.Circle(
                    (bbcc_clust[0], bbcc_clust[1]),
                    1.2 * radius_deg,  # outer sky annulus
                    fill=False,
                    edgecolor="gray",
                    lw=1,
                )
...
def display_ccd_apphot():
    ...
    for data1, label, color in zip(
        (image_nopipesky.flat[pipeskypix], image_nonewsky.flat[newskypix]),
    ...
    ax[1].plot(
        apphot["RCEN"],
        apphot["PIPESKYFLUX"] / apphot["SKYAREA"],
        label="Pipeline",
        color=setcolors[0],
    )
    ax[1].plot(
        apphot["RCEN"],
        apphot["NEWSKYFLUX"] / apphot["SKYAREA"],
        label="Custom",
        color=setcolors[1],
    )
...
def _display_ccdmask_and_sky(ccdargs):
    ...
    image, hdr = fitsio.read(
        os.path.join(galaxydir, "{}-ccddata-grz.fits.fz".format(galaxy)),
        header=True,
        ext=key,
    )
    ...
    for thisax, data, title in zip(
        ax.flat,
        (image, newmask, pipesky, newsky),
        ("Image", "Custom Mask", "Pipeline Sky", "Custom Sky"),
    ):
...
def _display_ellipse_sbprofile(ellipsefit, skyellipsefit={}, minerr=0.0, cosmo=None, png=None, verbose=True):
    ...
    if ellipsefit["success"]:
        ...
        if False:
            ...
            ax1.plot(
                ellipsefit[refband].sma * refpixscale,
                ellipsefit[refband].eps,
                zorder=1,
                alpha=0.9,
                lw=2,
            )
        ...
        if False:
            ...
            ax2.plot(
                ellipsefit[refband].sma * refpixscale,
                np.degrees(ellipsefit[refband].pa),
                zorder=1,
                alpha=0.9,
                lw=2,
            )
