...
def forced_phot(galaxy, survey, srcs, cat, band, log):
    ...
    print(
        "Build the model and residual image in band {}.".format(band),
        flush=True,
        file=log,
    )
...
def custom_coadds(
    ...
    for data, suffix in zip(
        (imgs, imgs_central, mods, mods_nocentral, resids),
...
def download(sample, pixscale=0.396, bands="gri", clobber=False):
    ...
    for onegal in sample:
        ...
        size_mosaic = 2 * legacyhalos.misc.cutout_radius_kpc(
            pixscale=pixscale,  # [pixel]
            redshift=onegal["Z"],
