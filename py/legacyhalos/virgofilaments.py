...
def missing_files(args, sample, size=1, clobber_overwrite=None):
    ...
    if len(itodo) > 0:
        ...
        if len(idx) < size:  # can happen in corner cases or with 1 rank
            # todo_indices = np.array_split(_todo_indices, size) # unweighted
            todo_indices = np.array_split(
                _todo_indices[np.argsort(weight)], size
            )  # unweighted but sorted
        else:
...
def mpi_args():
    ...
    parser.add_argument(
        "--d25min", default=0.0, type=float, help="Minimum diameter (arcmin)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rebuild-unwise", action="store_true", help="Rebuild the unWISE mosaics."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-galex-ceres",
        action="store_true",
        help="Do not use Ceres solver to perform GALEX forced photometry.",
    )
    parser.add_argument(
        "--no-unwise",
        action="store_false",
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="galex",
        help="Do not build GALEX coadds or do forced GALEX photometry.",
    )
    parser.add_argument(
        "--no-cleanup",
        action="store_false",
    ...
    parser.add_argument(
        "--build-catalog", action="store_true", help="Build the final catalog."
    )
...
def read_sample(
    ...
    samplefile = os.path.join(
        legacyhalos.io.legacyhalos_dir(), "vf_north_{}_main_groups.fits".format(version)
    )
...
def _build_multiband_mask(
    ...
    def tractor2mge(indx, factor=1.0, minsize=15.0):
        ...
        default_majoraxis = (
            tractor.diam_init[indx] * 60 / 2 / filt2pixscale[refband]
        )  # [pixels]
        ...
        objmask_center = ellipse_mask(
            mgegalaxy.xmed,
            mgegalaxy.ymed,  # object pixels are True
            majoraxis10,
            majoraxis10 * (1 - mgegalaxy.eps),
            np.radians(mgegalaxy.theta - 90),
            xobj,
            yobj,
        )
    ...
    for ii, central in enumerate(galaxy_indx):
        ...
        )  # , plot=True) ; plt.savefig('cosmo-www/tmp/junk-mge.png')
        ...
        for filt in bands:
            ...
            if len(satindx) == 0:
                ...
                if satmask.shape != satimg.shape:
                    thissatmask = (
                        resize(
                            thissatmask, satmask.shape, mode="edge", anti_aliasing=False
                        )
                        > 0
                    )
        ...
        for filt in bands:
            ...
            if satmask.shape != thismask.shape:
                _satmask = (
                    resize(satmask, thismask.shape, mode="edge", anti_aliasing=False)
                    > 0
                ) == 1.0
                ...
                _centralmask = (
                    resize(
                        _centralmask, thismask.shape, mode="edge", anti_aliasing=False
                    )
                    > 0
                ) == 1.0
...
def read_multiband(
    ...
    galaxy_indx = np.hstack(
        [
            np.where((tractor.ref_id == sid) * (tractor.ref_cat != "  "))[0]
            for sid in sample[REFIDCOLUMN]
        ]
    )
    ...
    if write_mask:
        ...
        for igal, galid in enumerate(galaxy_id):
            maskfile = os.path.join(
                galaxydir,
                "{}-{}-ellipsemask-{}.fits.gz".format(galaxy, filesuffix, galid),
            )
            ...
            if os.path.isfile(maskfile) and not clobber:
                ...
                for ifilt, filt in enumerate(data["bands"]):
                    mask = data["{}_masked".format(filt.lower())][igal].mask.astype(
                        np.int8
                    )
                    fitsio.write(
                        tmpfile,
                        mask,
                        header=data["{}_header".format(filt.lower())],
...
def rebuild_unwise(
    ...
    if not os.path.isfile(
        os.path.join(galaxydir, "{}-custom-coadds.isdone".format(galaxy))
    ):
        print(
            "Custom coadds not done; skipping rebuilding WISE coadds.",
            flush=True,
            file=log,
        )
    ...
    if os.path.isfile(
        ...
        os.path.join(galaxydir, "{}-custom-resid-W1W2.jpg".format(galaxy))
    ):
        print(
            "No WISE data to process; skipping rebuilding WISE coadds.",
            flush=True,
            file=log,
        )
        return 1, stagesuffix
    ...
    if not os.path.isdir(backupdir):
        ...
        ok = _copyfile(
            os.path.join(galaxydir, "{}-{}-tractor.fits".format(galaxy, filesuffix)),
            os.path.join(backupdir, "{}-{}-tractor.fits".format(galaxy, filesuffix)),
            clobber=True,
        )
        if not ok:
        ...
        for band in ("W1", "W2", "W3", "W4"):
            for imtype in ("image", "invvar"):
                ok = _copyfile(
                    os.path.join(
                        galaxydir,
                        "{}-{}-{}-{}.fits.fz".format(galaxy, filesuffix, imtype, band),
                    ),
                    os.path.join(
                        backupdir,
                        "{}-{}-{}-{}.fits.fz".format(galaxy, filesuffix, imtype, band),
            ...
            ok = _copyfile(
                os.path.join(
                    galaxydir, "{}-{}-model-{}.fits.fz".format(galaxy, filesuffix, band)
                ),
                os.path.join(
                    backupdir, "{}-{}-model-{}.fits.fz".format(galaxy, filesuffix, band)
                ),
                clobber=True,
            )
            if not ok:
        ...
        for imtype in ("image", "model", "resid"):
            ok = _copyfile(
                os.path.join(
                    galaxydir, "{}-{}-{}-W1W2.jpg".format(galaxy, filesuffix, imtype)
                ),
                os.path.join(
                    backupdir, "{}-{}-{}-W1W2.jpg".format(galaxy, filesuffix, imtype)
                ),
                clobber=True,
            )
            if not ok:
    ...
    cmd = cmd.format(
        legacypipe_dir=os.getenv("LEGACYPIPE_CODE_DIR"),
        galaxy=galaxy,
        galaxydir=galaxydir,
        unwise_dir=os.environ.get("UNWISE_COADDS_DIR"),
        ra=onegal[racolumn],
        dec=onegal[deccolumn],
        width=width,
        pixscale=pixscale,
        threads=nproc,
        outdir=galaxydir,
    ...
    if err != 0:
        ...
        for imtype, suffix in zip(
            ...
            ok = _copyfile(
                ...
                os.path.join(
                    galaxydir, "{}-{}-{}-W1W2.jpg".format(galaxy, filesuffix, suffix)
                ),
                clobber=True,
            )
            if not ok:
...
def build_catalog_one(galaxy, galaxydir, fullsample, refcat="R1", verbose=False):
    ...
    for onegal in fullsample:
        ...
        ellipsefile = os.path.join(
            galaxydir, "{}-custom-ellipse-{}.fits".format(galaxy, refid)
        )
...
def build_catalog(
    sample, fullsample, nproc=1, refcat="R1", verbose=False, clobber=False
):
    import time
    ...
    outfile = os.path.join(
        ...
        "virgofilaments-{}-legacyphot.fits".format(version),
    )
    if os.path.isfile(outfile) and not clobber:
...
def build_htmlpage_one(
        ...
        if os.path.isfile(tractorfile):
            ...
            if prefix == "custom":
                ...
                for ii, sid in enumerate(sample[REFIDCOLUMN]):
                    ww = np.where(
                        (tractor["ref_cat"] != "  ") * (tractor["ref_id"] == sid)
                    )[0]
                    if len(ww) > 0:
                        wt.append(ww)
                        ws.append(ii)
    ...
    def _html_image_mosaics(html):
        ...
        pngfile, thumbfile = "{}-custom-montage-grz.png".format(
            galaxy1
        ), "thumb-{}-custom-montage-grz.png".format(galaxy1)
        html.write(
