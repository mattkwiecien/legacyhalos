    ...
    for band in bands:
        for imtype in ["model"]:
            ...
            ok = _copyfile(
                ...
                os.path.join(
                    output_dir,
                    "{}-{}-{}-{}.fits.fz".format(galaxy, stagesuffix, imtype, band),
                ),
                clobber=clobber,
                missing_ok=missing_ok,
            )
    ...
    if unwise:
        for band in ("W1", "W2", "W3", "W4"):
            ...
            ok = _copyfile(
                ...
                os.path.join(
                    output_dir,
                    "{}-{}-model-{}.fits.fz".format(galaxy, stagesuffix, band),
                ),
                clobber=clobber,
                missing_ok=missing_ok,
            )
    ...
    if galex:
        ...
        for imtype, suffix in zip(("galex", "galexmodel", "galexresid"), ("image", "model", "resid")):
            ok = _copyfile(
                ...
                os.path.join(
                    output_dir,
                    "{}-{}-{}-FUVNUV.jpg".format(galaxy, stagesuffix, suffix),
                ),
                clobber=clobber,
                missing_ok=missing_ok,
            )
    ...
    if write_wise_psf:
        ...
        for band in (1, 2, 3, 4):
            ...
            if band == 4:
                ...
                xx, yy = np.meshgrid(
                    np.arange(0.0, pw - 0.51, 0.5, dtype=np.float32),
                    np.arange(0.0, ph - 0.51, 0.5, dtype=np.float32),
                )
                xx = xx.ravel()
                yy = yy.ravel()
