...
def srcs2image(cat, wcs, band="r", allbands="grz", pixelized_psf=None, psf_sigma=1.0):
    ...
    if pixelized_psf is None:
        vv = psf_sigma**2
        psf = tractor.GaussianMixturePSF(1.0, 0.0, 0.0, vv, vv, 0.0)
    else:
        psf = pixelized_psf
...
def get_area(cen, nside=256, qaplot=False):
    ...
    for dr, release in zip(("dr6.0", "dr7.1"), (6000, 7000)):
        ...
        keep = (
            (pixmap["FRACAREA"][thesepix] > 0)
            * (pixmap["PSFDEPTH_G"][thesepix] > 0)
            * (pixmap["PSFDEPTH_R"][thesepix] > 0)  # p10depth[0]) *
            * (pixmap["PSFDEPTH_Z"][thesepix] > 0)  # p10depth[1]) *  # p10depth[2])
        )
...
def ellipse_matrix(r, e1, e2):
    ...
    theta = np.atleast_1d(np.arctan2(e2, e1) / 2.0)
    ct = np.cos(theta)
    st = np.sin(theta)
