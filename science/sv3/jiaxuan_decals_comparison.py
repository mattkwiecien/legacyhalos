...
def SBP_single(
    ...
    if show_dots:
        ax1.errorbar((x**0.25), y, yerr=asymmetric_error, color="k", alpha=0.2, fmt="o", capsize=4, capthick=1, elinewidth=1)
    ...
    if vertical_line is not None:
        ...
        for k, pos in enumerate(vertical_line):
            ax1.axvline(x=pos**0.25, ymin=0, ymax=1, color="gray", linestyle=style_list[k], linewidth=3, alpha=0.75)
        plt.ylim(ylim)
...
def main():
    ...
    for i, my_fit in enumerate(ellipse_cat[start:]):
        ...
        ax_lower.text(0.05, 0.9, "Link", url=(url % (id[:4], id, id)), color="black", fontsize=18, transform=ax_lower.transAxes)
