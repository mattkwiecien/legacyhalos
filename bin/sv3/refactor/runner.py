...
import legacyhalos.io as legacy_io
...
class MpiRunner(ABC):
    ...
    def __init__(self):
        ...
        self.basedir = legacy_io.legacyhalos_dir()
        self.datadir = legacy_io.legacyhalos_data_dir()
        self.htmldir = legacy_io.legacyhalos_html_dir()
        self.step = None
        self.args = None
    ...
    def setup(self):
        ...
        # Get appropriate step
        step = StepGenerator.get(self.args, sample)

        # Setup runner with this step
        self.step = step
    ...
    def get_sample(self):
        ...
        if self.rank == 0:
            ...
            print("$LEGACYHALOS_DIR={}".format(self.basedir))
            print("$LEGACYHALOS_DATA_DIR={}".format(self.datadir))
            print("$LEGACYHALOS_HTML_DIR={}".format(self.htmldir))
    ...
    def run_step(self):
        ...
        # Guard against running serial programs in parallel.
        if self.rank > 0 and self.step.is_serial:
            print("Cannot run serial step in parallel. Exiting.")
        elif self.step.is_serial:
            self.step.run()
...
class RunnerGenerator(object):
    ...
    @classmethod
    def get(self, runner_type: str) -> MpiRunner:
        if runner_type.lower() == 'sv3':
            return Sv3MpiRunner()
        else:
            raise NotImplementedError()

class Sv3MpiRunner(MpiRunner):
    
    def __init__(self):
        super.__init__()

    def _get_args(self):
        from legacyhalos.sv3 import mpi_args
        return mpi_args()

    
    def _get_sample(self):
        from legacyhalos.sv3 import read_sample
        sample = read_sample(
    ...
    if rank == 0:
        ...
        print("{} left to do: {} / {} divided across {} rank(s)."
            .format(suffix.upper(), ntodo, len(sample), size), flush=True)
    ...
    if len(groups[rank]) == 0:
        ...
        print("{} for all {} galaxies on rank {} are complete!"
            .format(suffix.upper(), len(sample), rank), flush=True)
    ...
    print("Starting {} {} on rank {} with {} cores on {}"
        .format(len(groups[rank]), suffix.upper(), rank, args.nproc, time.asctime()),flush=True)
    ...
    for count, ii in enumerate(groups[rank]):
        ...
        print("Rank {:03d} ({} / {}): {} (index {})"
            .format(rank, count + 1, len(groups[rank]), galaxydir, ii),flush=True)
    ...
    if rank == 0:
        print("Finished {} {} at {} after {:.3f} minutes"
            .format(ntodo,suffix.upper(), time.asctime(), (time.time() - tall) / 60,), flush=True)
